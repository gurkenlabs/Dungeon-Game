...
public final class GameManager {
  ...
  public static void init() {
    ...
    Game.audio().playMusic(Resources.sounds().get("background-music.mp3"));
  }

  private static void loaded(Environment e) {
    if (!Game.world().environment().getMap().getName().equals("atrium")) {
      spawnEnemy(e);
    }
  ...
  public static void spawnStoryChar(int State) {
    switch (State) {
      case 0:
        return;
      case 1:
        Optional<Spawnpoint> summon = Optional.ofNullable(
          Game.world().environment().getSpawnpoint("n"));
        ...
        p.setName("marker");
        p.setLocation(StoryChar.instance().getCenter().getX() - 17,
          StoryChar.instance().getCenter().getY() - 50);
